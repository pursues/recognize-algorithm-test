...
from fireSmoke import DEFAULT_IMAGE_PATH
from fireSmoke import DEFAULT_MODEL_PATH
from fireSmoke import FEATURE_CONFIG
from fireSmoke import OUTPUTS_DIR
from fireSmoke import FireSmokeDetector


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="测试 fireSmoke 目录下的火灾烟雾识别模型。")
    ...
    parser.add_argument("--window-name", default="Fire and Smoke Detection", help="实时识别窗口标题")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=OUTPUTS_DIR,
...
def main() -> None:
    ...
    expected_labels = set(FEATURE_CONFIG.expected_labels)
    firesmoke_detections = [
        detection for detection in prediction.detections if detection.label in expected_labels
    ]
    ...
    result = {
        ...
        "firesmoke_detections": [
            ...
            for detection in firesmoke_detections
        ],
        "detected": bool(firesmoke_detections),
